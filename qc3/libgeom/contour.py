...
def build_parallel(p, radius, recursionlimit=6):
  ...
  if check_parallel(p, proposed_segment, radius) or recursionlimit <= 0:
    ...
    return (
      build_parallel(sd[:4], radius, recursionlimit - 1)
...
def get_join_segment(startpoint, endpoint, radius, jointype, miter_limit=MITER_LIMIT):
  """
...
def create_stroke_outline(
  ...
  for item in [fw_segments, bw_segments]:
    join_segs(item, radius, linejoin, miter_limit, path[2] == sk2const.CURVE_CLOSED)

  # Set caps for unclosed paths
  if not path[2] == sk2const.CURVE_CLOSED:
    fw_segments.insert(
      0, get_cap_segment(bw_segments[-1][-1], fw_segments[0][0], captype)
    )
    bw_segments.insert(
      0, get_cap_segment(fw_segments[-1][-1], bw_segments[0][0], captype)
    )
...
def make_path(segments, close=1):
  ...
  for seg in segments:
    ...
    while len(seg) >= 4:
      points.append(deepcopy([seg[1], seg[2], seg[3], sk2const.NODE_CUSP]))
      last_point = seg[3]
      seg = seg[3:]
...
def stroke_to_curve(paths, stroke_style):
  ...
  for path in paths:
    outlines = []
    fw, bw = create_stroke_outline(path, width / 2.0, joint, caps, miter_limit)
