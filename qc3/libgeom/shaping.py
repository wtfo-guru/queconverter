...
def equal_points(p0, p1):
  return round(p0[0], PRECISION) == round(p1[0], PRECISION) and round(
    p0[1], PRECISION
  ) == round(p1[1], PRECISION)
...
class StrokeHitSurface:
  ...
  def check_point(self, point):
    ...
    self.ctx.set_line_width(line_width - 0.04)
    dash = []
...
class PathObject:
  ...
  def split_seg(self, base_point, point, t=0.5):
    if is_bezier(point):
      ...
      new_point = add_points(mult_point(base_point, (1.0 - t)), mult_point(point, t))
...
def equal(point1, point2):
  return round(point1[0], PRECISION) == round(point2[0], PRECISION) and round(
    point1[1], PRECISION
  ) == round(point2[1], PRECISION)
    ...
    for j in range(i + 1, len(approx_paths)):
      for path1, approx_path1, rect1 in approx_paths[i]:
        for path2, approx_path2, rect2 in approx_paths[j]:
          if not path1.obj_id == path2.obj_id and is_bbox_overlap(rect1, rect2):
            for p in range(1, len(approx_path1)):
              ...
              for q in range(1, len(approx_path2)):
                ...
                if equal(p0, p2):
                  cp = p0
                elif equal(p0, p3) or equal(p1, p2) or equal(p1, p3):
                  cp = None
                else:
...
def intersect_and_join(paths1, paths2, rule=INTERSECT_RULE):
  ...
  for item in new_paths:
    if rule == INTERSECT_RULE and not contained(objs[abs(item.obj_id - 1)], item):
      continue
    if rule == FUSION_RULE and contained(objs[abs(item.obj_id - 1)], item):
      continue
