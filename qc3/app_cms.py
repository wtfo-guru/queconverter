...
class AppColorManager(DefaultColorManager):
  """Represents full-featured Color Manager for UniConverter application"""

  app: qc3const.AppHandle

  def __init__(self, app: qc3const.AppHandle) -> None:
    """Creates AppColorManager object for provided application instance

    :param app: (UCApplication) UniConverter application handle
    """
    self.app = app
  ...
  def _update_opts(self) -> None:
    ...
    if self.proofing:
      self.flags |= qc3const.cmsFLAGS_SOFTPROOFING
    if self.gamutcheck:
      self.flags |= qc3const.cmsFLAGS_GAMUTCHECK
    if config.cms_bpc_flag:
      self.flags |= qc3const.cmsFLAGS_BLACKPOINTCOMPENSATION
    if config.cms_bpt_flag:
      self.flags |= qc3const.cmsFLAGS_PRESERVEBLACK
