...
def get_glyph_cache(font_name: str, char: str) -> tp.Optional[qc3const.PathsType]:
  """Returns cached glyph paths

  :param font_name: (str) font name
  :param char: (str) character
  :return: (qc3const.PathsType) glyph paths
  """
  if font_name in GLYPH_CACHE and char in GLYPH_CACHE[font_name]:
    return deepcopy(GLYPH_CACHE[font_name][char])


def set_glyph_cache(font_name: str, char: str, glyph: qc3const.PathsType):
  """Sets cached glyph paths

  :param font_name: (str) font name
  :param char: (str) character
  :param glyph: (qc3const.PathsType) glyph paths
  """
...
def create_layout(ctx: cairo.Context = CTX) -> qc3const.PyCapsule:
...
def get_font_description(
  text_style: list, check_nt: bool = False
) -> qc3const.PyCapsule:
...
def set_layout(
  ...
  width: int,
  text_style: list,
  markup: tp.Optional[list] = None,
  layout: qc3const.PyCapsule = PANGO_LAYOUT,
) -> None:
  """Sets layout markup
...
def set_glyph_layout(
  text: str,
  width: int,
  text_style: list,
  markup: tp.Optional[list] = None,
  text_range: tp.Optional[list] = None,
  check_nt: bool = False,
  layout: qc3const.PyCapsule = PANGO_LAYOUT,
) -> float:
...
def layout_path(
  ctx: cairo.Context = CTX, layout: qc3const.PyCapsule = PANGO_LAYOUT
) -> None:
...
def get_char_positions(
  size: int, layout: qc3const.PyCapsule = PANGO_LAYOUT
...
def get_layout_size(layout: qc3const.PyCapsule = PANGO_LAYOUT) -> tp.Tuple[int, int]:
...
def get_layout_bbox(layout: qc3const.PyCapsule = PANGO_LAYOUT) -> tp.List[float]:
  """Returns text layout size
