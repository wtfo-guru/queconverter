...
def get_glyphs(ctx, layout_data, text, width, text_style, markup):
  ...
  for item in text:
    ...
    vpos = core.set_glyph_layout(
      item, width, text_style, markup, text_range, True, layout
    )
    if vpos:
      for index in range(*text_range):
        x, y, w, h, base_line, byte_index = layout_data[index]
        dh = (y - base_line) * vpos
        layout_data[index] = (x, y + dh, w, h, base_line + dh, byte_index)
    ...
    if os.name == "nt":
      m00 *= 0.1
      m11 *= 0.1
    matrix = cairo.Matrix(m00, 0.0, 0.0, m11, layout_data[i][0], layout_data[i][1])
...
def get_rtl_glyphs(
  ...
  for item in layout_data:
    ...
    layout = core.create_layout(ctx)
    vpos = core.set_glyph_layout(
      txt, width, text_style, markup, text_range, True, layout
    )
    if vpos:
      for index in range(*text_range):
        x, y, w, h, base_line, byte_index = log_layout_data[index]
        dh = (y - base_line) * vpos
        log_layout_data[index] = (x, y + dh, w, h, base_line + dh, byte_index)
    core.layout_path(ctx, layout)
    cpath = ctx.copy_path()
    m00 = 1.0
    m11 = -1.0
...
def get_text_paths(orig_text, width, text_style, markup):
  ...
  if text_style[5]:
    ...
    if not rtl_flag and not bidi_flag:
      ...
      glyphs = get_glyphs(ctx, layout_data, text, width, text_style, markup)
    else:
  ...
  layout_bbox = [0.0, layout_data[0][1], float(w), layout_data[0][1] - float(h)]
