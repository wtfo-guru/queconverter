...
def cms_open_profile_from_file(profile_path: str) -> qc3const.PyCapsule:
...
def cms_open_profile_from_bytes(profile_bytes: bytes) -> qc3const.PyCapsule:
    """Returns a handle to lcms profile wrapped as a Python object.
    The handle doesn't require to be closed after usage because
    on object delete operation Python calls native cms_close_profile()
    function automatically.
...
def cms_create_srgb_profile() -> qc3const.PyCapsule:
...
def cms_create_cmyk_profile() -> qc3const.PyCapsule:
...
def cms_create_lab_profile() -> qc3const.PyCapsule:
...
def cms_create_gray_profile() -> qc3const.PyCapsule:
...
COLOR_FUNC_MAP = {
    qc3const.COLOR_RGB: (cms_create_srgb_profile, get_srgb_profile_resource, save_srgb_profile),
    qc3const.COLOR_CMYK: (cms_create_cmyk_profile, get_cmyk_profile_resource, save_cmyk_profile),
    qc3const.COLOR_LAB: (cms_create_lab_profile, get_lab_profile_resource, save_lab_profile),
    qc3const.COLOR_GRAY: (cms_create_gray_profile, get_gray_profile_resource, save_gray_profile),
    qc3const.COLOR_DISPLAY: (cms_create_display_profile, get_display_profile_resource, save_display_profile),
}


def cms_create_default_profile(colorspace: str) -> tp.Optional[qc3const.PyCapsule]:
...
INTENTS: tp.List[int] = [qc3const.INTENT_PERCEPTUAL,
                         qc3const.INTENT_RELATIVE_COLORIMETRIC,
                         qc3const.INTENT_SATURATION,
                         qc3const.INTENT_ABSOLUTE_COLORIMETRIC]


def cms_create_transform(in_profile: qc3const.PyCapsule, in_mode: str,
                         out_profile: qc3const.PyCapsule, out_mode: str,
                         intent: int = qc3const.INTENT_PERCEPTUAL,
                         flags: int = qc3const.cmsFLAGS_NOTPRECALC) -> qc3const.PyCapsule:
    """Returns a handle to lcms transformation wrapped as a Python object.

    :param in_profile: (qc3const.PyCapsule) valid lcms profile handle
    :param in_mode: (str) valid lcms or PIL mode
    :param out_profile: (qc3const.PyCapsule) valid lcms profile handle
...
def cms_create_proofing_transform(in_profile: qc3const.PyCapsule, in_mode: str,
                                  out_profile: qc3const.PyCapsule, out_mode: str,
                                  proof_profile: qc3const.PyCapsule,
                                  intent: int = qc3const.INTENT_PERCEPTUAL,
                                  pintent: int = qc3const.INTENT_RELATIVE_COLORIMETRIC,
                                  flags: int = qc3const.cmsFLAGS_SOFTPROOFING) -> qc3const.PyCapsule:
    """Returns a handle to lcms transformation wrapped as a Python object.
    ...
    :param out_profile: (qc3const.PyCapsule) valid lcms profile handle
    :param out_mode: (str) valid lcms or PIL mode
    :param proof_profile: (qc3const.PyCapsule) valid lcms profile handle
    :param intent: (int) integer constant (0-3) of transform rendering intent
...
def cms_do_transform(transform: qc3const.PyCapsule, inbuff: tp.List[int], outbuff: tp.List[int]) -> None:
    """Transform color values from inputBuffer to outputBuffer using provided
    lcms transform handle.

    :param transform: (qc3const.PyCapsule) valid lcms transformation handle
...
def cms_do_bitmap_transform(transform: qc3const.PyCapsule, image: Image.Image,
    ...
    if image.mode not in qc3const.IMAGE_COLORSPACES:
        raise CmsError('Unsupported image type: %s' % image.mode)

    if in_mode not in qc3const.IMAGE_COLORSPACES:
        raise CmsError('Unsupported in_mode type: %s' % in_mode)

    if out_mode not in qc3const.IMAGE_COLORSPACES:
...
def cms_get_profile_name(profile: qc3const.PyCapsule) -> str:
    """Returns profile name

    :param profile: (qc3const.PyCapsule) valid lcms profile handle
    :return: profile name string
    """
    return _lcms2.getProfileName(profile).decode('cp1252').strip()


def cms_get_profile_info(profile: qc3const.PyCapsule) -> str:
...
def cms_get_profile_copyright(profile: qc3const.PyCapsule) -> str:
