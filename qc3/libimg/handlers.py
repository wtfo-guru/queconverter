...
from qc3.cms import val_255
from qc3.libcairo import image_to_surface
...
class ImageHandler(object):
  ...
  def _get_saver_fmt(self, image):
    return TIFF_FMT if image.mode == qc3const.IMAGE_CMYK else PNG_FMT
  ...
  def set_images_from_str(self, bitmap_str=None, alpha_str=None):
    self.set_images(self._str2image(bitmap_str), self._str2image(alpha_str))
  ...
  def load_from_images(self, cms, image, alpha=None):
    ...
    if image.mode == "P" and "transparency" in image.info:
      image = image.convert(qc3const.IMAGE_RGBA)
    LOG.debug("Image mode %s", image.mode)

    if not alpha and image.mode.endswith("A"):
      alpha = image.split()[-1]
    if image.mode == qc3const.IMAGE_GRAY_A:
      image = image.convert(qc3const.IMAGE_GRAY)

    if image.mode not in qc3const.SUPPORTED_CS:
      if image.mode != qc3const.IMAGE_RGBA:
        profile = None
      image = image.convert(qc3const.IMAGE_RGB)
    LOG.debug("Image mode %s", image.mode)

    if image.mode not in qc3const.SUPPORTED_CS[1:]:
      profile = None
    ...
    if image.mode in [qc3const.IMAGE_RGB, qc3const.IMAGE_LAB]:
      style[3] = deepcopy(cfg.default_rgb_image_style)
    self.pixmap.style = style

    if alpha:
      if alpha.mode == "P":
        alpha = alpha.convert(qc3const.IMAGE_RGBA)
  ...
  def extract_bitmap(self, filepath):
    ext = ".tiff" if self.bitmap.mode == qc3const.IMAGE_CMYK else ".png"
...
class DrawableImageHandler(ImageHandler):
  def convert_duotone_to_image(self, cms, cs=None):
    ...
    fg_cs = bg_cs = qc3const.IMAGE_RGB

    if self.bitmap.mode == qc3const.IMAGE_MONO:
      raw_image = raw_image.convert(qc3const.IMAGE_GRAY)
    size = raw_image.size

    if cs == qc3const.IMAGE_CMYK:
      fg = tuple(cms.get_cmyk_color255(fg)) if fg else ()
      bg = tuple(cms.get_cmyk_color255(bg)) if bg else ()
      fg_cs = bg_cs = cs
    elif cs == qc3const.IMAGE_RGB:
    ...
    return (fg_img, fg_alpha) if fg else None, (bg_img, bg_alpha) if bg else None

  def get_display_image(self, cms, proofing=False):
    image = self.bitmap
    if image.mode in qc3const.DUOTONES:
      if image.mode == qc3const.IMAGE_MONO:
        image = image.convert(qc3const.IMAGE_GRAY)
      size = image.size
      fg = self.pixmap.style[3][0]
      bg = self.pixmap.style[3][1]
      ...
      if fg:
        fg_img = Image.new(qc3const.IMAGE_RGB, size, fg)
      if bg:
        bg_img = Image.new(qc3const.IMAGE_RGB, size, bg)
    ...
    if proofing and image.mode != qc3const.IMAGE_CMYK:
      image = cms.convert_image(image, qc3const.IMAGE_CMYK)
    return cms.get_display_image(image)

  def _get_surface(self, cms, proofing=False, stroke_mode=False):
    if stroke_mode:
      gray_image = self.bitmap.convert(qc3const.IMAGE_GRAY)
      rgb_image = gray_image.convert(qc3const.IMAGE_RGB)
    else:
    ...
    if self.alpha and rgb_image.mode == qc3const.IMAGE_RGB:
...
class EditableImageHandler(DrawableImageHandler):
  def copy(self, pixmap=None):
    ...
    hdl.set_images(
      self.bitmap.copy() if self.bitmap else None,
  ...
  def invert_image(self, cms):
    if self.bitmap.mode == qc3const.IMAGE_MONO:
      image = ImageOps.invert(self.bitmap.convert(qc3const.IMAGE_GRAY))
      bitmap = image.convert(qc3const.IMAGE_MONO)
    elif self.bitmap.mode == qc3const.IMAGE_CMYK:
      image = cms.convert_image(self.bitmap, qc3const.IMAGE_RGB)
      inv_image = ImageOps.invert(image)
      bitmap = cms.convert_image(inv_image, qc3const.IMAGE_CMYK)
    elif self.bitmap.mode == qc3const.IMAGE_LAB:
      image = cms.convert_image(self.bitmap, qc3const.IMAGE_RGB)
      inv_image = ImageOps.invert(image)
      bitmap = cms.convert_image(inv_image, qc3const.IMAGE_LAB)
    else:
      bitmap = ImageOps.invert(self.bitmap)
    self.set_images(bitmap)

  def convert_image(self, cms, colorspace):
    if self.bitmap.mode in qc3const.DUOTONES:
      if colorspace not in qc3const.DUOTONES:
        bitmap = self.get_display_image(cms)
        alpha = None
        if self.alpha and bitmap.mode == qc3const.IMAGE_RGB:
          alpha = self.alpha
        elif bitmap.mode == qc3const.IMAGE_RGBA:
          alpha = bitmap.split()[-1]
          bitmap = bitmap.convert(qc3const.IMAGE_RGB)
        if colorspace != qc3const.IMAGE_RGB:
      ...
      if colorspace == qc3const.IMAGE_MONO:
        bitmap = cms.convert_image(self.bitmap, qc3const.IMAGE_GRAY)
