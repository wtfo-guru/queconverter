...
from qc3 import qc3const, libgeom, cms, sk2const
from qc3.formats.svg import svg_colors
from qc3.formats.xml_.xml_model import XMLObject, XmlContentText
from qc3.libgeom import add_points, sub_points, mult_point
...
def parse_svg_color(sclr, alpha=1.0, current_color=""):
  ...
  if sclr[0] == "#":
    if "icc-color" in sclr:
      ...
      if len(vals) == 5:
        ...
        if color_vals and len(color_vals) == 4:
          return [qc3const.COLOR_CMYK, color_vals, alpha, ""]
    ...
    try:
      ...
      clr = [qc3const.COLOR_RGB, vals, alpha, ""]
    except:
      pass
  elif sclr[:4] == "rgb(":
    ...
    if len(vals) == 3:
      ...
      clr = [qc3const.COLOR_RGB, decvals, alpha, ""]
  else:
...
def parse_svg_text(objs):
  ...
  for obj in objs:
    if obj.is_content():
      ...
      if "sodipodi:role" in obj.attrs and obj.attrs["sodipodi:role"].strip() == "line":
        ret += "\n"
  ret = ret.lstrip()
  if ret and ret[-1] == "\n":
    ret = ret[:-1]
