...
from qc3 import events, msgconst
from qc3 import qc3const
from qc3.utils import fsutils
from qc3.utils.fs import get_file_extension
...
def _get_loader(pid):
  ...
  loader = None
  try:
    loader_mod = import_module("qc3.formats." + pid)
...
def _get_saver(pid):
  ...
  saver = None
  try:
    saver_mod = import_module("qc3.formats." + pid)
...
def _get_checker(pid):
  ...
  checker = None
  try:
    checker_mod = import_module("qc3.formats." + pid)
...
def get_loader_by_id(pid):
  ...
  if not loader:
    msg = "Loader is not found for id %s" % (qc3const.FORMAT_NAMES[pid],)
...
def get_saver(path, experimental=False, return_id=False):
  ...
  ext = get_file_extension(path)
  saver = None
  sv_formats = [] + qc3const.SAVER_FORMATS

  msg = "Start to search saver by file extension %s" % (ext.__str__())
  events.emit(events.MESSAGES, msgconst.INFO, msg)

  if experimental:
    sv_formats += qc3const.EXPERIMENTAL_SAVERS
