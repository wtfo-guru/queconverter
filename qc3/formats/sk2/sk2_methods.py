...
from qc3 import _, qc3const, libgeom
...
class SK2_Methods:
  ...
  def set_default_page_size(self, width, height):
    ...
    fmt = _("Custom size")
    size = (width, height)
    orient = qc3const.PORTRAIT
    if width > height:
      orient = qc3const.LANDSCAPE
  ...
  def get_active_layers(self, page):
    return [
      layer for layer in self.get_visible_layers(page) if self.is_layer_editable(layer)
    ]
  ...
  def set_layer_rgba_color(self, layer, color):
    ...
    layer.style[1][2] = [qc3const.COLOR_RGB, color[:-1], color[-1], "", ""]
  ...
  def set_guide_rgb_color(self, color):
    self.set_guide_color([qc3const.COLOR_RGB, color, 1.0, "", ""])
  ...
  def fit_page_to_image(self, page, objs=None):
    ...
    if objs:
      ...
      orient = qc3const.PORTRAIT if h > w else qc3const.LANDSCAPE
