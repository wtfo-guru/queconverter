...
from qc3 import libcairo, libgeom, sk2const
...
class CairoRenderer:
  ...
  def render_container(self, ctx, obj):
    ...
    if container.style[1] and not self.contour_flag and container.style[1][7]:
      ctx.new_path()
    ...
    if container.style[1] and not self.contour_flag and not container.style[1][7]:
      ctx.new_path()
      self.process_stroke(ctx, None, container.style)
      ctx.append_path(container.cache_cpath)
      ctx.stroke()
  ...
  def render_image(self, ctx, obj):
    ...
    matrix = cairo.Matrix(zoom * m11, -zoom * m12, -zoom * m21, zoom * m22, x0, y0)
    ctx.set_matrix(matrix)

    ctx.set_source_surface(surface)
    if zoom * abs(m11) > 0.98:
  ...
  def process_fill(self, ctx, obj):
    ...
    if fill[1] == sk2const.FILL_SOLID:
      ...
      if not obj.fill_trafo:
        obj.fill_trafo = [] + sk2const.NORMAL_TRAFO
        obj.fill_trafo = obj.fill_trafo[:4] + [obj.cache_bbox[0], obj.cache_bbox[3]]
      pattern_fill = fill[2]
      ...
      canvas_trafo = libcairo.get_trafo_from_matrix(canvas_matrix)
      zoom = canvas_trafo[0]
      if zoom * abs(obj.fill_trafo[0]) > 0.98:
        ctx.get_source().set_filter(cairo.FILTER_NEAREST)
