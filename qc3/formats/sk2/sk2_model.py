...
class Document(DocumentObject):
  ...
  def __init__(self, config):
    ...
    self.styles = {
      "Default Style": deepcopy(
        [self.config.default_fill, self.config.default_stroke, [], []]
      )
    }
    self.resources = {}
  ...
  def update(self):
    if self.metainfo is None:
      self.metainfo = deepcopy(
        [
          self.config.doc_author,
    ...
    if "Default Text Style" not in self.styles:
      self.styles["Default Text Style"] = deepcopy(
        [self.config.default_text_fill, [], self.config.default_text_style]
      )
    elif len(self.styles["Default Text Style"]) == 5:
      self.styles["Default Text Style"] += [
        True,
      ]
...
class Pages(DocumentObject):
  ...
  def __init__(self, config, parent=None):
    ...
    size = deepcopy(qc3const.PAGE_FORMATS[fmt])
...
class Layer(StructuralObject):
  ...
  def update(self):
    ...
    stroke = self.style[1]
    if stroke:
      stroke[2] = [qc3const.COLOR_RGB, self.color[:3], self.color[3], ""]
...
class Guide(StructuralObject):
  ...
  def __init__(self, config, parent=None, pos=0.0, orient=qc3const.HORIZONTAL):
    self.config = config
    self.parent = parent
    self.cid = GUIDE
    self.position = pos
  ...
  def update_bbox(self):
    if self.childs:
      ...
      for child in self.childs[1:]:
        self.cache_bbox = libgeom.sum_bbox(self.cache_bbox, child.cache_bbox)
...
class PrimitiveObject(SelectableObject):
  ...
  def to_curve(self):
    ...
    curve.paths = deepcopy(self.paths if self.is_curve else self.cache_paths)
...
class Rectangle(PrimitiveObject):
  ...
  def get_initial_paths(self):
    return libgeom.get_rect_paths(self.start, self.width, self.height, self.corners)
  ...
  def get_midpoints(self):
    return [
      [self.start[0], self.start[1] + self.height / 2.0],
...
class Polygon(PrimitiveObject):
  ...
  def get_initial_paths(self):
    return libgeom.get_polygon_paths(
      self.corners_num, self.angle1, self.angle2, self.coef1, self.coef2
    )
...
class Curve(PrimitiveObject):
  ...
  def arrows_to_curve(self):
    ...
    if arrow_paths and self.style[1]:
      ...
      arrow_fill_style = [sk2const.FILL_NONZERO, sk2const.FILL_SOLID, color]
      arrow_style = [arrow_fill_style, [], [], []]
    if arrow_paths:
      curve = Curve(self.config, self.parent, paths=arrow_paths, style=arrow_style)
    return curve
        ...
        def get_vector(path, trafo):
          ...
          for point in path[1]:
            ...
            if not p0 == p1:
              ...
              break
          return [libgeom.apply_trafo_to_point(p, trafo) for p in (p0, p1)]
...
class Text(PrimitiveObject):
  ...
  def get_glyphs(self):
    glyphs, points, data, bbox, cl = libgeom.get_text_glyphs(
      self.get_text(), self.width, self.style[2], self.markup
    )
...
class Pixmap(PrimitiveObject):
  ...
  def get_size(self):
    width = float(self.size[0]) * qc3const.px_to_pt
    height = float(self.size[1]) * qc3const.px_to_pt
    return width, height

  def get_initial_paths(self):
    width, height = self.get_size()
    return libgeom.get_rect_paths([0, 0], width, height, [] + sk2const.CORNERS)
  ...
  def get_resolution(self):
    ...
    v_dpi = int(round(qc3const.in_to_pt / m11))
    h_dpi = int(round(qc3const.in_to_pt / m22))
    return h_dpi, v_dpi
