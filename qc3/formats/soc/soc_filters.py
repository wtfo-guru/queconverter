...
from qc3.formats.generic_filters import AbstractXMLLoader, AbstractSaver
from qc3.formats.soc.soc_const import (
  SOC_COLOR_TAG,
  SOC_COLOR_NAME_ATTR,
  SOC_COLOR_VAL_ATTR,
  SOC_PAL_TAG,
...
class SOC_Loader(AbstractXMLLoader):
  ...
  def start_element(self, name, attrs):
    ...
    if name == SOC_COLOR_TAG:
      self.model.colors.append(
        [attrs._attrs[SOC_COLOR_VAL_ATTR], attrs._attrs[SOC_COLOR_NAME_ATTR]]
      )
...
class SOC_Saver(AbstractSaver):
  ...
  def do_save(self):
    ...
    for item in self.model.colors:
      self.writeln(
        ' <%s %s="%s" %s="%s"/>'
        % (SOC_COLOR_TAG, SOC_COLOR_NAME_ATTR, item[1], SOC_COLOR_VAL_ATTR, item[0])
      )
