...
from qc3 import _, qc3const
from qc3 import events, msgconst
from qc3.utils import fs, fsutils, mixutils
...
class ModelPresenter(object):
  ...
  cid = 0
  model_type = qc3const.GENERIC_MODEL
  config = None
  doc_dir = ""
  doc_file = ""
  doc_id = ""
  ...
  def load(self, filename=None, fileptr=None):
    ...
    model_name = qc3const.FORMAT_NAMES[self.cid]
  ...
  def close(self):
    ...
    self.model = None
    model_name = qc3const.FORMAT_NAMES[self.cid]
    self.send_ok(_("<%s> document model is destroyed for %s") % (model_name, filename))
...
class TextModelPresenter(ModelPresenter):
  model_type = qc3const.TEXT_MODEL


class TaggedModelPresenter(ModelPresenter):
  model_type = qc3const.TAGGED_MODEL


class BinaryModelPresenter(ModelPresenter):
  model_type = qc3const.BINARY_MODEL
