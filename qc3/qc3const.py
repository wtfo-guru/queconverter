...
ColorType = tp.List[
  tp.Union[str, tp.Union[ColorValueType, tp.List[ColorValueType]], float]
]

# POINT: [x, y]
PointType = tp.List[float]
ScreenPointType = tp.Union[
  tp.Tuple[float, float], tp.Tuple[int, int], tp.List[int], tp.List[float]
]
...
MODEL_LOADERS = [CGM, SK2] # [SVGZ, CDR, CMX, CCX, XAR, WMF, PLT, SK1, SK, FIG, CGM,  DST ]  # CDT,
BITMAP_LOADERS = [] # [PNG, JPG, PSD, XCF, JP2, TIF, GIF, BMP, PCX, PPM, XBM, XPM, WEBP]
PALETTE_LOADERS = [SOC] # [SKP, GPL, SCRIBUS_PAL, CPL, COREL_PAL, ASE, ACO, JCW]
EXPERIMENTAL_LOADERS = [] # [MD, RIFF, XML ]

MODEL_SAVERS = [SVG, SK2] # [SVGZ, PLT, PDF, CDR, CMX, CCX, SK1, SK, CGM, FIG, DST]
BITMAP_SAVERS = [] # [PNG]
PALETTE_SAVERS = [SOC] # [SKP, GPL, SCRIBUS_PAL, CPL, COREL_PAL, ASE, ACO, JCW]
...
PAGE_FORMATS_MORE = {
  ...
  "C5": ("mm", 1.0 * mm_to_pt),
  "C6": ("mm", 1.0 * mm_to_pt),
  "Letter": ("in", 0.1 * in_to_pt),
  "Legal": ("in", 0.1 * in_to_pt),
  "Ledger": ("in", 0.1 * in_to_pt),
  "Executive": ("in", 0.1 * in_to_pt),
  ...
  "Envelope C4": ("in", 0.1 * in_to_pt),
  "Envelope C5": ("in", 0.1 * in_to_pt),
  "Envelope C6": ("in", 0.1 * in_to_pt),
  "Envelope E65/DL": ("in", 0.1 * in_to_pt),
